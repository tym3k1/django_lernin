...
# >web request ---- returns web respose-
@api_view(['GET', 'POST'])
def countries_list(request):
    if request.method == 'GET':
        ...
        countreis_serializer = CountriesSerializer(countries, many=True)
        return JsonResponse(countreis_serializer.data, safe=False)
        # 'safe=False' for obj serialization

    elif request.method == 'POST':
        countries_data = JSONParser().parse(request)
        countreis_serializer = CountriesSerializer(data=countries_data)
        if countreis_serializer.is_valid():
            countreis_serializer.save()
            return JsonResponse(countreis_serializer.data, status=status.HTTP_201_CREATED)
        return JsonResponse(countreis_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
